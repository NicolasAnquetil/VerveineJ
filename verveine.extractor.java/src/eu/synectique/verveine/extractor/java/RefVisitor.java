...
/**
 * This visitor offers methods to recover FamixEntities from the main JDT Entities<br>
 * It is used in refVisitors but also in defVisitors, for example, the VisitorClassMethodDef
 * assumes packages are already created (by VisitorPackageDef) and thus recovers the corresponding FamixNamespaces<p>
 ...
 * the recover methods here are named after the visited entities:s visit(CompilationUnit node) becomes visitCompilationUnit(CompilationUnit node).
 */
public class RefVisitor extends ASTVisitor {
	...
	public RefVisitor(JavaDictionary dico) {
		this.dico = dico;
	...
	/**
	 * See field {@link RefVisitor#anonymousSuperTypeName}<br>
	...
	/**
	 * See field {@link RefVisitor#anonymousSuperTypeName}
	 */
	...
	/**
	 * Local type: same as {@link RefVisitor#visitClassInstanceCreation(ClassInstanceCreation)}, 
